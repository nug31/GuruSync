...
export function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface font-body">
      ...
      <aside className="h-full w-72 fixed left-0 top-0 hidden lg:flex flex-col bg-inverse-surface z-50 shadow-xl">
        <div className="flex flex-col h-full p-6">
          <div className="mb-8 flex items-center gap-3 px-2">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-primary to-tertiary flex items-center justify-center text-white shadow-md shadow-primary/30">
              <span className="material-symbols-outlined text-[22px]">qr_code_2</span>
            </div>
            <div>
              <h1 className="text-xl font-extrabold text-white tracking-tight font-display">GuruSync</h1>
              <p className="text-[10px] uppercase tracking-wider text-inverse-on-surface/50 font-bold">Portal Kepegawaian</p>
            </div>
          </div>
          ...
          <div className="mt-auto pt-6 border-t border-white/10 flex flex-col gap-1">
            <button className="text-inverse-on-surface/50 hover:text-white hover:bg-white/5 px-4 py-2.5 rounded-xl flex items-center gap-3 text-sm text-left transition-colors">
              <span className="material-symbols-outlined text-[18px]">help</span>
              <span>Bantuan</span>
            </button>
            <button
              onClick={() => signOut()}
              className="text-inverse-on-surface/50 hover:text-error hover:bg-error/10 px-4 py-2.5 rounded-xl flex items-center gap-3 text-sm text-left transition-colors font-medium"
            >
      ...
      <header className="fixed top-0 right-0 w-full lg:w-[calc(100%-18rem)] z-40 bg-white/80 backdrop-blur-md border-b border-outline-variant flex justify-between items-center px-6 lg:px-10 h-20">
        ...
        <div className="hidden lg:block">
          <span className="text-xs font-bold uppercase tracking-wider text-on-surface-variant/70">Portal Kepegawaian & Administrasi</span>
        </div>

        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 pl-4 lg:border-l border-outline-variant">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold text-on-surface leading-none">{userName || profile?.email}</p>
              <span className="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-primary-fixed text-on-primary-fixed-variant border border-primary-fixed-dim uppercase tracking-wider">
                {userRole}
              </span>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 w-full z-40 lg:hidden bg-white/95 backdrop-blur-md border-t border-outline-variant flex justify-around items-center h-16 px-4 shadow-lg">
        <button
          onClick={() => setView('dashboard')}
          className={`flex flex-col items-center justify-center py-1 transition-colors ${view === 'dashboard' ? 'text-primary font-bold' : 'text-on-surface-variant/60 hover:text-on-surface'}`}
        >
        ...
        <button
          onClick={() => setView('teachers')}
          className={`flex flex-col items-center justify-center py-1 transition-colors ${view === 'teachers' ? 'text-primary font-bold' : 'text-on-surface-variant/60 hover:text-on-surface'}`}
        >
        ...
        <button
          onClick={() => setView('permissions')}
          className={`flex flex-col items-center justify-center py-1 transition-colors ${view === 'permissions' ? 'text-primary font-bold' : 'text-on-surface-variant/60 hover:text-on-surface'}`}
        >
        ...
        {isAdmin && (
          <button
            onClick={() => setView('admins')}
            className={`flex flex-col items-center justify-center py-1 transition-colors ${view === 'admins' ? 'text-primary font-bold' : 'text-on-surface-variant/60 hover:text-on-surface'}`}
          >
